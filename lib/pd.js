...
app.on('option ready', function(options){
	var object = options.pd.flags;
	var array = option.pd.flags = [];
	...
	for (var i = 0, l = object.path.length; i < l; ++i){
		array.push('-path', option.dirname + object.path[i]);
	}
...
//			services.emit('set', service, state);
	...
	*/
	// console.log(require(option.dirname + 'pd/manager'));
});
